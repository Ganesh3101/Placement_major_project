...
class offcampus_filter_list : AppCompatActivity() {
    @SuppressLint("MissingInflatedId")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_offcampus_list)

        val recyclerview= findViewById<RecyclerView>(R.id.offcampus_company_select)
        ...
        for(i in 1..145) {
            ...
            listing_data.addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if (snapshot.exists()) {
                        if(snapshot.child("Off Campus").getValue() != "") {

                            var name = snapshot.child("Name of Student").getValue()
                            var company_name = snapshot.child("Off Campus").getValue()
